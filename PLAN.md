...
- P4.6 精确 jar 的新世界和 ETF/RTF/C2ME 客户端验收已闭环。P4.7-0A 自动基线现已覆盖 cache、
  ...
  SoA tile、bounded Priority-Flood/adaptive-flow/stream-power 和统一 synthetic 同图视觉审查均已完成。
  下一步是 P4.7-0B 四组合客户端/JFR；在该门禁和真实 terrain finalist 对照完成前，不得宣布候选胜出、
  重写 production cache 或接入 `EndDensity`。
...
- `EndTerrainComposer` 当前仍使用一个低频 selector 按权重区间选择 plains/hills/plateau/volcano 等层，并在相邻区间做 cross-fade；该路径只保留为 legacy。受控 `REGION_PLANNED` 已完成 AREA-only terrain region、多候选 AREA morphology 与独立三段曲线 RIDGE overlay，但仍缺稳定 resource key、侵蚀排水和删除中心抬升后的真实客户端验收。按大陆中心距离抬升整体地形的方案已否决并移除。`COMPACT` 火山草稿冻结，不属于当前生产路线，因此自动测试通过不代表地貌已经达到发布质量。
...
- [>] `RTF_ADVANCED`、terrain region、地貌族、shape-aware 山脉/火山和附属群岛已统一移动到 P4，避免继续把“大陆算法”和“完整地表质量”拆成互不相干的补丁。
...
- `0.2.0-alpha.2`：核心地貌族、稳定区域变体和 eligibility 完成，替换“连续抖动噪声”
  原型。
- `0.2.0-alpha.3`：有限山脉、附属群岛和海岸层级完成；中心抬升穹顶不再属于路线，火山仍为后续独立里程碑。
...
#### P4.5：R7 uplift（已否决并移除）

- [x] 2026-07-23 否决“按大陆中心距离抬升整体地形”的设计：它会把大陆扭曲成中心穹顶，
  并把宏观地形压力错误地转嫁给未来 3D 河流/水位系统。
- [x] 移除 `EndTerrainUpliftRuntime`、`EndTerrainSignalBuffer.uplift()`、`getUplift`、
  `TerrainPreviewMode.UPLIFT`、调色板/语言 key 和 `RTF_UPLIFT_EXPERIMENTAL`；
  `REGION_PLANNED` 高度不再消费 continent center distance。
- [x] 2026-07-23 删除后自动门禁顺序通过：相关定点测试、完整 `:common:test`、NeoForge/Fabric
  编译、`:verifyReleaseArtifacts --no-daemon`、`git diff --check` 和严格仓库卫生门禁；新 NeoForge
  jar SHA-256 为 `1AD2103C47441C891BABCECEDE8148F519245C899DFD5F569C752DD71C1CB134`。
- [ ] 删除后的新 jar 尚未进行真实客户端视觉回归；此前 P4.6 uplift smoke 证据只能作为历史快照，
  不能证明删除后的新地表效果。
...
- [x] 2026-07-26 完成 canonical primitive tile substrate：stable value key 覆盖 algorithm/world/runtime/
  bounds/terrain/tile/geometry，immutable SoA input artifact 固定 8 float + 2 int + 1 byte channel；worker-owned
  bounded cache 记录 hit/miss/build/eviction/owner swap、resident/build primitive peak。正负坐标、128/256
  block core、build failure、请求重排和 1/2/4/6 worker checksum 通过；4 个共享 key 的 duplicate builds
...
- [x] 2026-07-26 完成统一 synthetic 同图视觉门禁：固定 256-block core、4-block sampling、world-space
  fixture 和 delta/drainage 色标，对比 source、analytical、thermal、hydraulic 与 bounded flow。审查发现并
  修复 analytical 在 archipelago activation 为零时仍发布 drainage 的契约漏洞。当前淘汰 analytical-only
  和 thermal-only 成形路线；hydraulic 与 bounded flow 保留为实机 finalist，但尚未获选。证据见
  [`docs/reviews/P4_7_CANDIDATE_VISUAL_REVIEW_2026-07-26.md`](docs/reviews/P4_7_CANDIDATE_VISUAL_REVIEW_2026-07-26.md)。
