 ...
 *
 * Ported verbatim from ReTerraForged's NoiseUtil (MIT) into EndTerraForged
 * (LGPL-3.0-or-later). Lineage: TerraForged (dags) -> ReTerraForged
 * (raccoonman) -> EndTerraForged.
 *
 * Renamed to NoiseMath to avoid a clash with the existing minimal
 * endterraforged.util.NoiseUtil (which holds the general-purpose seed/clamp/
 * div helpers). This class owns the noise-specific lookup tables and gradient
 * math; the static initialiser and every numeric literal are preserved
 * byte-for-byte so Perlin/Simplex output stays identical to upstream.
 */
package endterraforged.world.noise;

/**
 * Noise lookup tables, hashing and gradient math shared by the noise module.
 *
 * <p>The {@code GRAD_2D}, {@code GRAD_2D_24}, {@code CELL_2D} and {@code SIN}
 * tables are precomputed at class-init and treated read-only afterwards, so
 * they are safe to read concurrently from any thread. All methods are
...
public final class NoiseMath {
    ...
    public static Vec2f cell(int seed, int x, int y) {
        return NoiseMath.CELL_2D[hash2D(seed, x, y) & 0xFF];
    }
    ...
    public static Vec2f coord2D(int seed, int x, int y) {
        ...
        hash = hash * hash * hash * 60493;
        hash ^= hash >> 13;
        return NoiseMath.GRAD_2D[hash & 0x7];
    }
    ...
    public static Vec2f coord2D_24(int seed, int x, int y) {
        ...
        return NoiseMath.GRAD_2D_24[selector24];
    }
    ...
    public static float sin(float r) {
        int index = (int)(r * NoiseMath.radToIndex) & NoiseMath.SIN_MASK;
        return NoiseMath.SIN[index];
    }
    ...
    static {
        ...
        SIN_BITS = 12;
        SIN_MASK = ~(-1 << NoiseMath.SIN_BITS);
        SIN_COUNT = NoiseMath.SIN_MASK + 1;
        radFull = 6.2831855f;
        degFull = 360.0f;
        radToIndex = NoiseMath.SIN_COUNT / NoiseMath.radFull;
        degToIndex = NoiseMath.SIN_COUNT / NoiseMath.degFull;
        SIN = new float[NoiseMath.SIN_COUNT];
        for (int i = 0; i < NoiseMath.SIN_COUNT; ++i) {
            NoiseMath.SIN[i] = (float)Math.sin((i + 0.5f) / NoiseMath.SIN_COUNT * NoiseMath.radFull);
        }
        for (int i = 0; i < 360; i += 90) {
            NoiseMath.SIN[(int)(i * NoiseMath.degToIndex) & NoiseMath.SIN_MASK] = (float)Math.sin(i * 3.141592653589793 / 180.0);
        }
    }
