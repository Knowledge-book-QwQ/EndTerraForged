...
5. `RTF_UPLIFT` 只移植纯大陆抬升部分，暂不移植其水位、河流和主世界地貌耦合。
...
“单一 selector + 通用 Perlin 层”的旧地貌编排。`RTF_UPLIFT`、hydraulic erosion 和
当前 RTF 开发分支中的 shape-aware 扩展在完成独立性能和末地语义验证前只能是实验能力。
...
- `UpliftContinentGenerator` 只保留 Voronoi gradient 与 centroid uplift 标量。
...
1. **稳定 seed 命名空间**：为 continent、region、family、variant、ridge、volcano、
   uplift、archipelago、erosion 和 climate 分配固定 salt。新增模块不得通过改变顺序让
   旧模块的 seed 漂移。
...
### 4.3 `UpliftContinentGenerator`：可选的内陆抬升
...
这部分可以给 ETF 提供比普通 noise 更清晰的“海岸低、内陆高”宏观骨架。

但 RTF 当前实现还写入 `cell.waterTable`，并与河流、湖泊、terracing 和主世界水位深度
耦合。ETF 第一阶段不得搬入这些部分。
...
它只作为 `EndTerrainComposer` 的宏观 relief envelope，不负责水位、河流或方块放置。
...
| `UpliftContinentGenerator` gradient/uplift | 末地化改造 | 保留宏观抬升，删除 water/river/lake |
...
flowchart TD
    ...
    X --> H
    S --> U["Optional uplift / archipelago"]
    U --> H
...
- 已接入 `ContinentAlgorithm`、`EndHeightmap` 与 `OuterContinentsContinent`，但默认仍为
  `LEGACY_RADIAL`。
- 旧 JSON 缺失 `algorithm` 时解码为 legacy；未实现的 advanced/uplift 选项明确拒绝。
...
### 阶段 R7：`RTF_UPLIFT_EXPERIMENTAL`

- 只移植平滑 Voronoi gradient 和 centroid uplift。
- 输出独立 uplift scalar，作为宏观 relief envelope。
- 不接 water table、river cache、lake 或 terracing。
- 经视觉和性能验收后再决定是否进入高质量默认。
...
- `RTF_UPLIFT` 不得在每个 Y 重新计算板块 gradient。
...
| uplift 携带主世界语义 | 只输出 scalar，不移植 water table/river |
...
7. 将 RIDGE 扩展为弯曲/多段山链，再依次接入非径向 uplift、附属群岛和 analytical erosion。
